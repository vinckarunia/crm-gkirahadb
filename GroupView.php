...
// Hitung jumlah anggota laki-laki dan perempuan dalam grup
$sSQLGender = "
    SELECT per_Gender, COUNT(*) AS total
    FROM person_per
    INNER JOIN person2group2role_p2g2r ON per_ID = p2g2r_per_ID
    WHERE p2g2r_grp_ID = $iGroupID
    GROUP BY per_Gender
";
$rsGender = RunQuery($sSQLGender);
...
while ($row = mysqli_fetch_assoc($rsGender)) {
    if ($row['per_Gender'] == 1) {
        $totalMale = $row['total'];
    } elseif ($row['per_Gender'] == 2) {
        $totalFemale = $row['total'];
    } elseif ($row['per_Gender'] == 0) {
        $totalUnassigned = $row['total'];
    }
}
$totalAll = $totalMale + $totalFemale + $totalUnassigned;
...
<div class="card">
    <div class="card-header with-border">
        <h3 class="card-title"><?= gettext('Group Members:') ?></h3><br>
        <ul class="list-unstyled">
            <li><strong><?= gettext('Male') ?>:</strong> <?= $totalMale ?></li>
            <li><strong><?= gettext('Female') ?>:</strong> <?= $totalFemale ?></li>
            <li><strong><?= gettext('Unassigned') ?>:</strong> <?= $totalUnassigned ?></li>
            <li><strong><?= gettext('Total') ?>:</strong> <?= $totalAll ?></li>
        </ul>
    </div>
    <div class="card-body">
