...
<!--Select Event Form -->
<form class="well form-horizontal" name="selectEvent" action="QRCheckin.php" method="POST">
    <div class="row">
...
if (!$CheckoutOrDelete &&  $EventID > 0) {
    ...
    <form class="well form-horizontal" method="post" action="QRCheckin.php" id="AddAttendees" data-toggle="validator"
          role="form">
        ...
        <div class="row">
            <div class="col-md-10 col-xs-12">
                <div class="card card-primary">
                    <div class="card-header">
                        <h3 class="card-title"><?= gettext('Add Attendees for Event'); ?></h3>
                    </div>
                    <div class="card-body">
                        <div class="form-group">
                            <div class="row align-items-center">
                                ...
                                <div class="col-md-6" id="person-id">
                                    <input type="number" class="form-control" id="child-id-input"
                                        placeholder="<?= gettext("Enter Person ID"); ?>" required tabindex="1">
                                </div>
                                <div class="col-md-6 text-center">
                                    <h5><strong><?= gettext("Current Time") ?></strong></h5>
                                    <div id="live-time" class="form-control-plaintext font-weight-bold text-primary" style="font-size:1.1em;">
                                        --
                                    </div>
                                    <hr>
                                    <h5><strong><?= gettext("Attending Person") ?></strong></h5>
                                    <div id="child-photo" class="d-inline-block"></div>
                                    <input type="hidden" name="CheckIn" value="1">
                                    <input type="text" class="form-control mt-2" id="child-name-display" disabled
                                        placeholder="<?= gettext("Person Name"); ?>">
                                </div>
                            </div>
                        </div>
                        <hr>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="text-right col-md-8 col-xs-4">
                                <a class="btn btn-primary" href="Checkin.php"><?= gettext('Manual Checkin'); ?></a>
...
if (
    ...
    <form class="well form-horizontal" method="post" action="QRCheckin.php" id="CheckOut" data-toggle="validator"
          role="form">
            ...
            <table id="checkedinTable" class="table data-table table-striped ">
                ...
                foreach ($eventAttendees as $per) {
                    ...
                    <tr>
                        ...
                        <td align="center">
                            <form method="POST" action="QRCheckin.php" name="DeletePersonFromEvent">
...
<script nonce="<?= SystemURLs::getCSPNonce() ?>" >
    ...
    $(document).ready(function() {
        // Ketika input ID kehilangan fokus atau tekan Enter
        $("#child-id-input").on("blur keypress", function(e) {
            if (e.type === "blur" || e.which === 13) {
                const id = $(this).val().trim();
                if (id === "") return;
                fetchPersonById(id, "#childDetails", "#child-id");
            }
        });
    });

    function fetchPersonById(id, detailSelector, hiddenInputSelector, autoSubmit = false) {
        $.ajax({
            url: window.CRM.root + '/api/person/' + id,
            type: 'GET',
            dataType: 'json',
            success: function (data) {
                const person = data.Person || data;
                if (person && person.Id) {
                    $("#AddAttendees #child-id").val(person.Id);

                    const name = person.FullName || `${person.FirstName || ''} ${person.MiddleName || ''} ${person.LastName || ''}`.trim();
                    $("#child-name-display").val(name);
                    $("#child-photo").html(`
                        <img src="${window.CRM.root}/api/person/${person.Id}/thumbnail"
                            class="img-circle img-fluid" alt="${name}"
                            style="max-width:80px; border:2px solid #ddd; border-radius:50%;">
                    `);
                } else {
                    $("#child-name-display").val('');
                    $("#child-photo").html('<div class="text-danger">Person not found</div>');
                }
            },
            complete: function() {
                if (autoSubmit) {
                    const form = document.getElementById("AddAttendees");
                    // pastikan CheckIn diset
            ...
            error: function () {
                $("#child-name-display").val('');
                $("#child-photo").html('<div class="text-danger">Error fetching person data</div>');
            }
        });
    }
    
    function onScanSuccess(decodedText, decodedResult) {
        const personId = parseInt(decodedText);
        if (!isNaN(personId)) {
            $("#child-id-input").val(personId);
            fetchPersonById(personId, "#childDetails", "#child-id", true);
        }
    }
