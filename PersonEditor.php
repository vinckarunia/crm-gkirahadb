    ...
    if (isset($_POST['PersonSubmit']) || isset($_POST['PersonSubmitAndAdd'])) {
        ...
        if (!$bErrorFlag) {
            ...
            if ($numCustomFields > 0) {
                ...
                $sSQL = '';
                $oldC14 = trim($aCustomData['c14'] ?? '');
                $newC14 = $_POST['c14'] ?? '';

                $updateUpdatedAt = ($oldC14 !== $newC14);
    ...
    <form method="post" action="PersonEditor.php?PersonID=<?= $iPersonID ?>" name="PersonEditor">
                        ...
                        <div class="col-md-2">
                            <label for="Title"><?= gettext('Title') ?></label>
                                <input type="text" name="Title" id="Title"
                                    ...
                                    class="form-control" placeholder="<?= gettext('Dr., Ir., Drs., ...') ?>">
                        </div>
                            ...
                            while ($rowCustomField = mysqli_fetch_array($rsCustomFields, MYSQLI_BOTH)) {
                                if ($rowCustomField['custom_Field'] === $customFieldKey) {
                                    ...
                                    if (AuthenticationManager::getCurrentUser()->isEnabledSecurity($aSecurityType[$custom_FieldSec])) {
                                        ?>
                                        <div class="form-group col-md-6">
                                            <label><?= gettext('Area of Expertise') ?></label>
                                            <?php
                                            if (array_key_exists($custom_Field, $aCustomData)) {
                                                $currentFieldData = trim($aCustomData[$custom_Field]);
                                            } else {
        ...
        <div class="card card-info clearfix">
            ...
            <div class="card-body">
                <div class="form-group">
                    <h5><span><?= gettext('Membership Details')?></span></h5><hr>
                    <div class="row mb-3">
                        <?php
                        $customFieldKey = 'c1';
                    ...
                    <hr><h5><span><?= gettext('Mutation Details')?></span></h5><hr>
                    <div class="row mb-3">
                        <?php
                        $customFieldKey = 'c8';
                            ...
                            while ($rowCustomField = mysqli_fetch_array($rsCustomFields, MYSQLI_BOTH)) {
                                if ($rowCustomField['custom_Field'] === $customFieldKey) {
                                    ...
                                    if (AuthenticationManager::getCurrentUser()->isEnabledSecurity($aSecurityType[$custom_FieldSec])) {
                                        ?>
                                        <div class="col-md-6">
                                            <label><?= gettext('Church Of Origin') ?></label>
                                            <?php
                                            if (array_key_exists($custom_Field, $aCustomData)) {
                                                $currentFieldData = trim($aCustomData[$custom_Field]);
                                            } else {
