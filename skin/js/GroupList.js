$(document).ready(function () {
    ...
    $("#table-filter").on("change", function () {
        const selected = $(this).val().toLowerCase().trim();
        if (selected === "sekolah minggu") {
            $("#assignSundaySchoolWrapper").show();
        } else {
            $("#assignSundaySchoolWrapper").hide();
        }
    });
    ...
    $("#assignSundaySchoolBtn").on("click", function () {
        if (confirm("Assign sunday school members?")) {
            const $btn = $(this);
            $btn.prop("disabled", true).text("Processing...");

            $.ajax({
                url: "autoAssignSundaySchool.php",
                method: "GET",
                dataType: "json",
                success: function (response) {
                    if (response.success) {
                ...
                complete: function () {
                    $btn.prop("disabled", false).text("Auto Assign Sunday School Members");
                }
            });
        }
    });
